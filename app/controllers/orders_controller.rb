...
class OrdersController < ApplicationController
  ...
  def complete
    ActiveRecord::Base.transaction do
      ...
      total_amount = calculate_total_amount cart_sub_total, coupon

      order = Order.create!(user_id: current_user.id, coupon_id: cart.coupon_id, total: total_amount)
      create_order cart_sub_total, total_amount, order, cart
    end
    redirect_to orders_url, notice: 'Order Complete Sucessfully'
  end

  def create_order(_cart_sub_total, _total_amount, order, cart)
    cart_items = CartProduct.where(cart_id: cart.id)
  ...
  def calculate_total_amount(cart_sub_total, coupon)
    cart_sub_total * (coupon.nil? ? 1 : (1 - coupon.discount))
  end
